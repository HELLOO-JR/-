...
local SETTINGS = {
    ...
    FruitSniper = {Enabled = false, AutoCollect = false, AutoStore = false, StorePercentage = 80, FruitBlacklist = {"Bomb-Bomb", "Spike-Spike", "Spin-Spin", "Spring-Spring", "Kilo-Kilo", "Smoke-Smoke", "Rocket-Rocket", "Falcon-Falcon", "Chop-Chop"}, AutoServerHop = false, ServerHopDelay = 3, AutoNotifier = false, AutoEat = false, EatBlacklist = {}},
    AutoRaid = {Enabled = false, Type = "Flame", AutoNext = false, AutoBuyChip = false, AutoAwaken = false, AutoRaidIfMaxed = false},
    AutoMastery = {Enabled = false, Weapon = "Current", AutoFarmMasteryIfMaxed = false},
    ...
    ESP = {
        Enabled = false,
        PlayerESP = false, FruitESP = false, EnemyESP = false, ChestESP = false,
        ESPDistance = 2000, ShowHealth = false, ShowDistance = false, ShowName = false,
        Tracers = false, BoxESP = false, Chams = false,
        Box3D = false, Skeleton = false, Line2D = false, Glow = false,
        HealthBar = false, Weapon = false, ViewLine = false,
        ItemESP = false, VehicleESP = false, Radar = false, ProjectileESP = false
    },
    Teleports = {FastTeleport = true, TweenSpeed = 250, AutoAvoidWater = true, WaterTP = true, SafeMode = false},
    Combat = {
        Godmode = false, InfiniteEnergy = false, NoStun = false, AutoDodge = false,
        KillAura = false, KillAuraRange = 30, FastAttack = false, FastAttackDuration = 2,
        FastAttackCooldown = 1, FastAttack200x = false,
        AutoBounty = false, AutoKenHaki = false, AutoBusoHaki = false, AutoSoru = false,
        AutoObservationV2 = false, AutoFlashStep = false, NoClip = false, HitboxExpander = false,
        HitboxSize = 15, AutoBlock = false, PerfectBlock = false
    },
    ...
    Misc = {AutoHaki = false, AutoRaceV4 = false, MirageFinder = false, ChestFarm = false, FPSBoost = false, WhiteScreen = false, NoFog = false, BlackScreen = false, LockFPS = 60, AutoBuyHaki = false, AutoBuyLegendarySword = false, AutoBuyGodhuman = false, AutoBuyRengoku = false, AutoBuyTrueTriple = false, AutoBuyHallowScythe = false, AutoBuyCursedDual = false, AutoBuySoulGuitar = false, AutoBuyMirrorFractal = false, AutoRace = "None"},
...
local function createFuturisticGUI()
    ...
    Title.Text = "⚡ RYZEN AI - BLOX FRUITS v2.0"
    ...
    addSectionLabel(TabPages["🏠 Home"], "Welcome to Ryzen AI v2.0")
    addSectionLabel(TabPages["🏠 Home"], "Inspired by Banana/Redz - All features OFF.")
    ...
    addToggle(TabPages["👹 Raid"], "Auto Raid if Maxed Fruit", SETTINGS.AutoRaid, "AutoRaidIfMaxed")
    ...
    addSectionLabel(TabPages["📜 Actions"], "Auto Race")
    addToggle(TabPages["📜 Actions"], "Auto Race", SETTINGS.Misc, "AutoRace")
...
local function espSystem()
    ...
    local function createViewLine(character, color)
        local head = character:FindFirstChild("Head")
        if not head then return end
        local direction = head.CFrame.LookVector * 50
        local endPos = head.Position + direction
        local line = createLine2D(
            Vector2.new(Camera:WorldToScreenPoint(head.Position).X, Camera:WorldToScreenPoint(head.Position).Y),
            Vector2.new(Camera:WorldToScreenPoint(endPos).X, Camera:WorldToScreenPoint(endPos).Y),
            color, 1
        )
    end
    ...
    local function updateRadar()
        ...
        radarDots = {}
        local radarSize = 150
        local radarCenter = Vector2.new(radarFrame.AbsolutePosition.X + radarSize/2, radarFrame.AbsolutePosition.Y + radarSize/2)
        local scale = 5
        ...
        while true do
            ...
            for _, player in ipairs(Players:GetPlayers()) do
                ...
                if SETTINGS.ESP.PlayerESP or SETTINGS.ESP.EnemyESP then
                    ...
                    if SETTINGS.ESP.ViewLine then createViewLine(character, color) end
...
local function initialize()
    print("[Ryzen AI v2.0] Blox Fruits Ultimate Script Loading...")
    notify("Ryzen AI v2.0", "Inspired by Banana/Redz - All features OFF.", 5)
    createFuturisticGUI()
    autoFarm()
    fruitSniper()
    autoBoss()
    ...
    print("[Ryzen AI v2.0] All systems online!")
end
